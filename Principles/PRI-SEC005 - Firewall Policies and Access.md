...
## Manage admin access appropriately
...
- Following least privilege at all times ensures that any changes are made consciously and appropriately
- Using a well-defined role-based access control (RBAC) method ensures that permissions required to manage the platform are appropriate
- Using a Just-In-Time access model, complemented with the least-privilege model, ensures that access is only provided when required for the specific task
- Separation of Duties must be followed to ensure that teams managing multiple environments only have access to the specific environment and service (Dev/UAT/Prod etc)
- Restricting access from DfE devices (not from BYO or 3rd party) ensures that the access is protected and secured appropriately


## General guidelines for firewall rules
...
### Enable access consciously and limit to least-access

- Open up consciously
- Limit access to only what\'s required

### Restrict access to services which could enable direct Internet access
